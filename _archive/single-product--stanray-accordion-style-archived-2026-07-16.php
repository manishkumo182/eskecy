...
<div class="sr-product">
    ...
    <div class="sr-layout">
        ...
        <!-- ═══ LEFT: Big main image + small gallery below ═ -->
        <?php
        $sr_main_img_id = $all_imgs[0] ?? null;
        $sr_sub_imgs    = array_slice( $all_imgs, 1 );
        ?>
        <div class="sr-images">
            <?php if ( $sr_main_img_id ) :
                $src  = wp_get_attachment_image_url( $sr_main_img_id, 'woocommerce_single' );
                $full = wp_get_attachment_image_url( $sr_main_img_id, 'full' );
                $alt  = get_post_meta( $sr_main_img_id, '_wp_attachment_image_alt', true ) ?: get_the_title();
            ?>
            <div class="sr-img-item sr-img-item--main">
            ...
            <?php if ( ! empty( $sr_sub_imgs ) ) : ?>
            <div class="sr-images__grid">
                <?php foreach ( $sr_sub_imgs as $img_id ) :
                ...
                ?>
                <div class="sr-img-item sr-img-item--sub">
                    <img src="<?php echo esc_url( $src ); ?>"
                         alt="<?php echo esc_attr( $alt ); ?>"
                         loading="lazy">
                    <button class="sr-zoom" type="button"
...
.sr-img-item--main { margin-bottom: 3px; }

/* Secondary gallery images — small grid below the main image */
.sr-images__grid {
    display: grid;
...
jQuery(function ($) {
    ...
    $('#srAtc').on('click', function () {
        ...
        /* WooCommerce's real ajax handler ("woocommerce_add_to_cart") resolves
           the parent product + attributes itself from a variation product_id —
           it does not read a separate variation_id/attribute field. */
        var postData = {
